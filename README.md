...
- Boost 1.85 (and probably any version before) does not directly compile with Visual Studio 2022 >= 17.10 (or probably, rather msvc-toolset >= 14.4), see [this boost bug](https://github.com/boostorg/boost/issues/914). For boost 1.85, there is now a patch included which is automatically applied. Should boost still fail to build, you might try the workaround of using an older (14.3x) runtime.
